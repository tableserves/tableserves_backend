...
class EmailOTPService {
  ...
  async sendEmailOTP(email, otp, purpose = 'verification') {
    try {
      ...
      // If transporter is not available (development mode), expose the OTP
      // only when running in development. In any other env we should never
      // print an OTP — that defeats the purpose of OTP auth.
      if (!this.transporter) {
        if (process.env.NODE_ENV === 'development') {
          logger.debug('Email OTP (development mode)', { to: email, otp, purpose });
          return {
            success: true,
        ...
        logger.error('Email transporter not configured in non-development env', {
          to: email,
          purpose
        });
        return {
