...
class SubscriptionService {
  ...
  static async getCurrentCounts(userId) {
    try {
      ...
      if (user.role === 'restaurant_owner') {
        ...
        // ✅ FIXED: Find restaurant by ownerId instead of user.restaurantId (which doesn't exist)
        ...
        // ✅ FIXED: Find zone by ownerId instead of user.zoneId (which doesn't exist)
      ...
      } else if (user.role === 'zone_shop') {
        // Count zone shop-specific items
        const ZoneShop = require('../models/ZoneShop');
        const MenuCategory = require('../models/MenuCategory');
        const MenuItem = require('../models/MenuItem');
        
        // ✅ FIXED: Find zone shop by managerId/staffId instead of user.zoneShopId (which doesn't exist)
        // Try to find zone shop where this user is a manager or staff member
   ...
   */
  static async checkSubscriptionLimits(userId, action, currentCounts = null, context = {}) {
    try {
      const user = await User.findById(userId).populate('subscription');
      ...
      switch (action) {
        ...
        case 'create_menu_item': {
          // `maxMenuItems` is a PER-CATEGORY limit, not a global one.
          // If a categoryId is provided in context, count items in that specific category.
          // Otherwise we cannot reliably enforce this here — let the route's middleware handle it.
          if (limits.maxMenuItems && context.categoryId) {
            const MenuItem = require('../models/MenuItem');
            const itemsInCategory = await MenuItem.countDocuments({ categoryId: context.categoryId });
            if (itemsInCategory >= limits.maxMenuItems) {
              return {
                allowed: false,
                reason: `Menu item limit reached (${limits.maxMenuItems} per category)`
              };
            }
          }
          break;
