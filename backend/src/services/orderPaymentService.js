...
class OrderPaymentService {
  ...
  verifyWebhookSignature(webhookData, signature) {
    try {
      ...
      logger.warn('Webhook signature verification threw', { error: error.message });
      return false;
    }
  }
  ...
  async handlePaymentCaptured(paymentEntity) {
    ...
    // Atomic, idempotent update: only flip the order to paid if it is not
    // already paid. Two concurrent webhooks (Razorpay retries) will race here
    // — only one will match `payment.status: { $ne: 'paid' }`, the other will
    // receive `null` and short-circuit. This prevents duplicate side effects
    // (notifications, audit rows, double order-confirmed events).
    const order = await Order.findOneAndUpdate(
      ...
      },
      {
        $set: {
          ...
          'payment.status': 'paid',
          'payment.method': paymentEntity.method || 'razorpay',
          'payment.paidAt': now,
          'payment.signatureVerified': true,
          'status': 'confirmed',
          'timing.orderConfirmed': now
        }
      },
      { new: true }
    );
    ...
    if (!order) {
      // Either the order doesn't exist, or it was already paid. Distinguish
      // the two so we log accurately and still return 200 (Razorpay will
      // retry on non-2xx, so duplicate webhooks must look "successful").
      ...
      logger.info('Payment already processed (idempotent webhook)', {
