...
const MenuItemSchema = new mongoose.Schema({
  ...
  // Simple isVeg field for quick access (maps to dietaryInfo.isVegetarian)
  isVeg: {
    type: Boolean,
    default: false,
    index: true
  },
  // Simple isSpicy field for quick access
  isSpicy: {
    type: Boolean,
    default: false
  },
...
// Pre-save middleware to sync isVeg with dietaryInfo.isVegetarian
MenuItemSchema.pre('save', function(next) {
  // Sync isVeg with dietaryInfo.isVegetarian
  if (this.isModified('isVeg')) {
    this.dietaryInfo.isVegetarian = this.isVeg;
  } else if (this.isModified('dietaryInfo.isVegetarian')) {
    this.isVeg = this.dietaryInfo.isVegetarian;
  }
  
  // Sync isSpicy with spicyLevel
  if (this.isModified('isSpicy')) {
    this.spicyLevel = this.isSpicy ? Math.max(this.spicyLevel, 1) : 0;
  } else if (this.isModified('spicyLevel')) {
    this.isSpicy = this.spicyLevel > 0;
  }
