...
// Instance method to check if Razorpay Route is active
ZoneShopSchema.methods.isRazorpayRouteActive = function() {
  return this.paymentConfig?.isPaymentReady || false;
};

// Instance method to get commission rate
ZoneShopSchema.methods.getCommissionRate = function() {
  if (this.paymentConfig?.commissionRate) {
    return this.paymentConfig.commissionRate;
  }
  
  // Default commission for zone shops
  return 10;
};

// Instance method to get payment status
ZoneShopSchema.methods.getPaymentStatus = function() {
  return {
