...
const getPublicTableServeStatistics = catchAsync(async (req, res) => {
  ...
  // Ensure we always return valid data even if no ratings exist
  const safeStats = {
    totalRatings: stats.totalRatings || 0,
    averageServiceRating: stats.averageServiceRating || 0,
    averageAppExperience: stats.averageAppExperience || 0,
    averageOrderingProcess: stats.averageOrderingProcess || 0,
    averagePaymentExperience: stats.averagePaymentExperience || 0,
    averageOverallSatisfaction: stats.averageOverallSatisfaction || 0,
...
const getPublicRecentTableServeRatings = catchAsync(async (req, res) => {
  ...
  const ratings = await TableServeRating.find({ isPublic: true })
    .sort({ createdAt: -1 })
    .limit(parseInt(limit))
    .select('customer serviceRating serviceFeedback createdAt')
    .lean();
  ...
  const formattedRatings = ratings.map(rating => ({
    ...
    serviceFeedback: rating.serviceFeedback || 'Great experience with TableServe!',
    submittedAt: rating.createdAt
  }));
