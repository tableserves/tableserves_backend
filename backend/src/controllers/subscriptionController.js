 ...
 * FIXED: Now returns actual subscription limits along with current counts
 */
const checkSubscriptionLimits = async (req, res) => {
  try {
    ...
    // Get user's subscription to fetch limits
    ...
    // Get subscription limits from the subscription object
    let subscriptionLimits = null;
    if (user.subscription) {
      subscriptionLimits = {
        maxTables: user.subscription.limits?.maxTables ?? null,
        maxVendors: user.subscription.limits?.maxVendors ?? null,
        maxShops: user.subscription.limits?.maxShops ?? null,
        maxCategories: user.subscription.limits?.maxCategories ?? null,
        maxMenuItems: user.subscription.limits?.maxMenuItems ?? null,
        maxUsers: user.subscription.limits?.maxUsers ?? 1,
        maxOrdersPerMonth: user.subscription.limits?.maxOrdersPerMonth ?? null,
        maxStorageGB: user.subscription.limits?.maxStorageGB ?? 1,
        unlimited: user.subscription.features?.unlimited || false
      };
    }
    ...
    res.status(200).json({
      ...
      data: {
        currentCounts,
        limits: subscriptionLimits, // ✅ NOW INCLUDES ACTUAL LIMITS
        limitCheck
      }
    });
  } catch (error) {
