...
if (process.env.RAZORPAY_KEY_ID && process.env.RAZORPAY_KEY_SECRET) {
  try {
    ...
    logger.error('Razorpay initialization failed', { error: error.message });
  }
}
  ...
  static verifyPlanPayment = catchAsync(async (req, res) => {
    ...
    if (user.role === 'restaurant_owner') {
      try {
        ...
        logger.error('❌ Error updating restaurant subscription:', error);
    ...
    if (user.role === 'zone_admin') {
      try {
        ...
        logger.error('❌ Error updating zone subscription:', error);
          ...
          if (!user.hasActivePlan || user.currentPlanId?.toString() !== payment.planId._id.toString()) {
            ...
            if (user.role === 'restaurant_owner') {
              try {
                ...
                logger.error('❌ Error updating restaurant subscription via webhook:', error);
            ...
            if (user.role === 'zone_admin') {
              try {
                ...
                logger.error('❌ Error updating zone subscription via webhook:', error);
