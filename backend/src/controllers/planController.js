...
class PlanController {
  ...
  static async getUserUsage(userId, planType) {
    // Delegate to SubscriptionService.getCurrentCounts, which already queries
    // the underlying models (Restaurant.tables, MenuCategory, MenuItem,
    // ZoneShop, etc.) based on the user's role. Keep the legacy field names
    // expected by planCanAccommodateUsage().
    const counts = await SubscriptionService.getCurrentCounts(userId);
    ...
    if (planType === 'zone') {
      usage.shops = counts.shops ?? 0;
      usage.vendors = counts.vendors ?? 0;
    }
