...
// @route   GET /api/v1/menu/public/:ownerType/:ownerId/variants
// @desc    Get variants for public viewing (customer-facing menu)
// @access  Public
router.get('/public/:ownerType/:ownerId/variants',
  ...getValidation('validateObjectId', 'ownerId'),
  handleValidation,
  MenuController.getPublicVariants || ((req, res) => res.status(501).json({ success: false, error: { message: 'Controller method not implemented' } }))
);

// @route   GET /api/v1/menu/public/shop/:shopId/variants
// @desc    Get shop variants for public viewing
// @access  Public
router.get('/public/shop/:shopId/variants',
  param('shopId').isMongoId().withMessage('Invalid shop ID format'),
  handleValidation,
  (req, res) => {
    req.params.ownerType = 'shop';
    req.params.ownerId = req.params.shopId;
    return MenuController.getPublicVariants(req, res);
  }
);

// @route   GET /api/v1/menu/public/restaurant/:restaurantId/variants
// @desc    Get restaurant variants for public viewing
// @access  Public
router.get('/public/restaurant/:restaurantId/variants',
  param('restaurantId').isMongoId().withMessage('Invalid restaurant ID format'),
  handleValidation,
  (req, res) => {
    req.params.ownerType = 'restaurant';
...
// @route   GET /api/v1/menu/public/restaurant/restaurant/:restaurantId/variants
// @desc    Get restaurant variants for public viewing (alternative URL format)
// @access  Public
router.get('/public/restaurant/restaurant/:restaurantId/variants',
  param('restaurantId').isMongoId().withMessage('Invalid restaurant ID format'),
  handleValidation,
  (req, res) => {
    req.params.ownerType = 'restaurant';
...
// ===== VARIANT MANAGEMENT ROUTES (Protected) =====

// Get variants
addValidationMiddleware(
  protectedRouter,
  'get',
  '/:ownerType/:ownerId/variants',
  [
    ...getValidation('validateObjectId', 'ownerId'),
    ...getValidation('validatePagination'),
    handleValidation
  ],
  MenuController.getVariants || ((req, res) => res.status(501).json({ success: false, error: { message: 'Controller method not implemented' } }))
);
...
addValidationMiddleware(
  ...
  [
    ...getValidation('validateObjectId', 'ownerId'),
    ...getValidation('createVariant'),
    handleValidation
  ],
  MenuController.createVariant || ((req, res) => res.status(501).json({ success: false, error: { message: 'Controller method not implemented' } }))
);
...
addValidationMiddleware(
  protectedRouter,
  'put',
  '/:ownerType/:ownerId/variants/:variantId',
  [
    ...getValidation('validateObjectId', 'ownerId'),
    param('variantId').isMongoId().withMessage('Invalid variant ID format'),
    ...getValidation('updateVariant'),
    handleValidation
  ],
  MenuController.updateVariant || ((req, res) => res.status(501).json({ success: false, error: { message: 'Controller method not implemented' } }))
);
...
addValidationMiddleware(
  ...
  MenuController.deleteVariant || ((req, res) => res.status(501).json({ success: false, error: { message: 'Controller method not implemented' } }))
);

// Zone Shop specific routes for variants
addValidationMiddleware(
  protectedRouter,
  'get',
  '/shop/:shopId/variants',
  [
    param('shopId').isMongoId().withMessage('Invalid shop ID format'),
    ...getValidation('validatePagination'),
    handleValidation
  ...
  (req, res) => {
    req.params.ownerType = 'shop';
    req.params.ownerId = req.params.shopId;
    return MenuController.getVariants(req, res);
  }
);
...
addValidationMiddleware(
  ...
  (req, res) => {
    req.params.ownerType = 'shop';
    req.params.ownerId = req.params.shopId;
    return MenuController.createVariant(req, res);
  }
);
...
addValidationMiddleware(
  protectedRouter,
  'put',
  '/shop/:shopId/variants/:variantId',
  [
    param('shopId').isMongoId().withMessage('Invalid shop ID format'),
    param('variantId').isMongoId().withMessage('Invalid variant ID format'),
    ...getValidation('updateVariant'),
  ...
  (req, res) => {
    req.params.ownerType = 'shop';
    req.params.ownerId = req.params.shopId;
    return MenuController.updateVariant(req, res);
  }
);
...
addValidationMiddleware(
  ...
  (req, res) => {
    req.params.ownerType = 'shop';
    req.params.ownerId = req.params.shopId;
    return MenuController.deleteVariant(req, res);
  }
);
