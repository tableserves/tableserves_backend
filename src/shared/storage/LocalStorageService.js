...
class LocalStorageService {
  ...
  static async getRestaurant(restaurantId) {
    try {
      // Use DataAccessLayer for consistency
      return await dataAccessLayer.getRestaurant(restaurantId);
    } catch (error) {
  ...
  static async saveRestaurants(restaurants) {
    try {
      // Use DataAccessLayer for consistency
      return await dataAccessLayer.saveRestaurants(restaurants);
    } catch (error) {
  ...
  static async addRestaurant(restaurant) {
    try {
      // Use DataAccessLayer for consistency
      const newRestaurant = await dataAccessLayer.addRestaurant(restaurant);
  ...
  static async updateRestaurant(restaurantId, updates) {
    try {
      // Use DataAccessLayer for consistency
      const updatedRestaurant = await dataAccessLayer.updateRestaurant(restaurantId, updates);
  ...
  static async deleteRestaurant(restaurantId) {
    try {
      // Use DataAccessLayer for consistency
      const deleted = await dataAccessLayer.deleteRestaurant(restaurantId);
  ...
  static async getRestaurantById(restaurantId) {
    try {
      ...
      logger.error('Error getting restaurant by ID', error, 'LocalStorageService');
      return null;
    }
  }
  ...
  static async getZones() {
    try {
      // Use DataAccessLayer for consistency
      const zones = await dataAccessLayer.getZones();
  ...
  static async exportData() {
    try {
      ...
      logger.error('Error exporting data', error, 'LocalStorageService');
      return {
        restaurants: [],
        zones: [],
        exportedAt: new Date().toISOString(),
  ...
  static async searchRestaurants(searchTerm, filters = {}) {
    try {
      const restaurants = await this.getRestaurants();
      ...
      logger.error('Error searching restaurants', error, 'LocalStorageService');
      return [];
    }
  }
  ...
  static async getZoneVendors(zoneId) {
    try {
      // Use API directly - no local storage
      const vendors = await ApiService.getZoneVendors(zoneId);
      return Array.isArray(vendors) ? vendors : [];
    } catch (error) {
      logger.error('Error loading zone vendors from API', error, 'LocalStorageService');
      return [];
    }
  }

  static async saveZoneVendors(zoneId, vendors) {
    try {
      // Cache vendors only - no local storage
      const success = await dataAccessLayer.saveVendors(zoneId, vendors);
      
      if (success) {
        logger.info('Zone vendors cached successfully', { zoneId, count: vendors.length }, 'LocalStorageService');
      }
      
      return success;
    } catch (error) {
      logger.error('Error caching zone vendors', error, 'LocalStorageService');
      return false;
    }
  }
