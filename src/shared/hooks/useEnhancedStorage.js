...
export function useEnhancedStorage(key, defaultValue = null, options = {}) {
  const {
    ...
    useRealTime = true,         // Whether to use real-time updates
  } = options;
  ...
  useEffect(() => {
    if (useRealTime) {
      cachingService.subscribeToUpdates(key);
      return () => cachingService.unsubscribeFromUpdates(key);
    }
  }, [key, useRealTime]);
...
export function useStorageMigration() {
  ...
  useEffect(() => {
    ...
    import('../storage/StorageMigrationService').then(({ default: service }) => {
  ...
  const triggerMigration = useCallback(async () => {
    try {
      setLoading(true);
      const { default: service } = await import('../storage/StorageMigrationService');
