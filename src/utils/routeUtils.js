...
export const generateRoutePath = (restaurantId, tableId, userId, zoneId, page) => {
    ...
    if (zoneId && tableId && userId) {
        return `/tableserve/zone/${zoneId}/table/${tableId}/user/${userId}/${page}`;
    }

    // Zone-based routes without user
    if (zoneId && tableId) {
        return `/tableserve/zone/${zoneId}/table/${tableId}/${page}`;
    }

    // Restaurant-based routes with user
    if (restaurantId && tableId && userId) {
        return `/tableserve/restaurant/${restaurantId}/table/${tableId}/user/${userId}/${page}`;
    }

    // Restaurant-based routes without user
    if (restaurantId && tableId) {
        return `/tableserve/restaurant/${restaurantId}/table/${tableId}/${page}`;
    }
