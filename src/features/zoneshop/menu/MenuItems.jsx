...
import { ZONE_PLANS } from '../../subscription/constants/plans';
...
const MenuItems = () => {
  ...
  // Plan restrictions integration for zone shops
  const {
    subscription,
    subscriptionLimits,
    currentCounts,
  ...
  const getLimit = (type) => {
    ...
    if (subscription?.key) {
      const planData = ZONE_PLANS[subscription.key];
  ...
  useEffect(() => {
    if (shopId) {
      console.log('Zone Shop MenuItems - Fetching data for shopId:', shopId);
      const limit = 1000;
      dispatch(fetchMenuCategories({ entityId: shopId, entityType: 'shop', limit }));
      dispatch(fetchMenuItems({ entityId: shopId, entityType: 'shop', limit }));
      dispatch(fetchMenuVariants({ entityId: shopId, entityType: 'shop', limit }));
      dispatch(fetchMenuModifiers({ entityId: shopId, entityType: 'shop', limit }));
    }
  }, [shopId, dispatch]);
  ...
  const addVariant = () => {
    setFormData(prev => ({
      ...
      variants: [...(prev.variants || []), {
        ...
        type: 'single',
        required: true,
        options: [{ name: '', price: '', isDefault: true }]
      }]
    }));
    ...
    try {
      ...
      if (selectedImageFile) {
        try {
          ...
          console.log('Zone Shop MenuItems - Image uploaded successfully:', uploadResult.url);
        } catch (uploadError) {
          console.error('Zone Shop MenuItems - Image upload failed:', uploadError);
      ...
      console.log('Zone Shop MenuItems - Submitting itemData:', itemData);
      ...
      if (result.type.endsWith('/fulfilled')) {
        ...
        console.log('Zone Shop MenuItems - Operation successful');
        resetForm();
      } else {
        const errorMessage = result.payload || 'Failed to save menu item';
        console.error('Zone Shop MenuItems - Operation failed:', errorMessage);
        safeToastError(errorMessage);
      }

    } catch (error) {
      console.error('Zone Shop MenuItems - Submit error:', error);
  ...
  const resetForm = () => {
    ...
    setLockedCategory(null); // Always clear the lock when the form is closed/reset
  };
  ...
  const confirmDelete = async () => {
    try {
      const result = await dispatch(deleteMenuItem({
        entityId: shopId,
        entityType: 'shop',
      ...
      if (result.type.endsWith('/fulfilled')) {
        console.log('Zone Shop MenuItems - Delete successful');
        safeToastSuccess('Item deleted successfully');
      } else {
        const errorMessage = result.payload || 'Failed to delete menu item';
        console.error('Zone Shop MenuItems - Delete failed:', errorMessage);
        safeToastError(errorMessage);
      }
    } catch (error) {
      console.error('Zone Shop MenuItems - Delete error:', error);
  ...
  const handleToggleAvailability = async (item) => {
    try {
      ...
      const result = await dispatch(updateMenuItem({ entityId: shopId, entityType: 'shop', itemId: itemId, itemData }));
      if (result.type.endsWith('/fulfilled')) {
  ...
  useEffect(() => {
    if (availableCategories.length > 0 && Object.keys(expandedCategories).length === 0) {
  ...
  // Handle add item with pre-selected category
  ...
  const handleAddItemToCategory = (categoryId) => {
    ...
    setLockedCategory(categoryId); // Lock the category so the user can't change it on this entry path
    setShowModal(true);
  };

  const handleAddMenuItem = () => {
    // Limit is PER CATEGORY, not global. Check whether AT LEAST ONE category still has space —
    // if every existing category is full, there's no point opening the form.
    // Note: if there are no categories yet, allow opening the form (user can create the category first).
    if (availableCategories.length > 0 && menuItemLimit !== '∞') {
      const anyCategoryHasSpace = availableCategories.some(cat => !isCategoryLimitReached(cat.id || cat._id));
      if (!anyCategoryHasSpace) {
        safeToastError(`All categories have reached the ${menuItemLimit} items per category limit. Please upgrade your plan or remove some items to add more.`);
        return;
      }
    }
                    ...
                    <div className="bg-theme-bg-secondary border-b border-theme-border-primary">
                      <div className="flex items-center justify-between p-4">
                        ...
                        <FeatureRestriction
                          ...
                          fallback={
                            <button 
                              onClick={() => navigate(`/zone/${zoneId}/shop/${shopId}/upgrade`)} 
                    ...
                    <div className="bg-theme-bg-secondary/50 rounded-xl p-5 border border-theme-border-primary">
                      ...
                      <div className="mt-4">
                        ...
                        <select
                          ...
                          required
                          disabled={!!lockedCategory} // When opened via per-category "Add Item", the category is fixed
                        >
