...
const Analytics = () => {
  ...
  useEffect(() => {
    try {
      ...
      // Filter raw orders for the ledger table and export
      const periodOrders = orders.filter(order => new Date(order.createdAt) >= boundaryDate)
        .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

      setFilteredOrdersList(periodOrders);

      // Calculate raw revenue values
      const selectedRevenue = periodOrders.reduce((total, order) => {
        let orderTotal = order.pricing?.total || order.totalAmount || order.grandTotal || order.total || 0;
  ...
  }, [restaurantId, timeRange, orders, liveOrders]);

  const calculateDailyRevenue = (orders, timeRange) => {
    const now = new Date();
    ...
    const dataMap = {};
    const dateToKeyMap = {}; // Track original dates for sorting
    ...
    orders.forEach(order => {
      ...
      if (orderDate >= startDate) {
        ...
        if (!dataMap[key]) {
          dataMap[key] = { date: key, revenue: 0, orders: 0, sortKey };
        }

        let orderRevenue = order.pricing?.total || order.totalAmount || order.grandTotal || order.total || 0;
  ...
  const formatDateAxis = (dateStr) => {
    // dateStr is already formatted as the label (Mon, Tue, 1, 2, Jan, Feb, etc.)
    // Just return it directly since calculateDailyRevenue now handles formatting
    return dateStr;
  };
  ...
  // Show loading only from ordersLoading (SingleRestaurantLayout handles auth loading)
  if (ordersLoading) {
    return (
    ...
    <SingleRestaurantLayout>
      ...
      <div className="w-full h-full flex flex-col gap-6 pb-12">
        ...
        <div className="admin-card p-6 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-theme-text-primary tracking-tight">Financial Intelligence</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Polished HTML/CSS Velocity Matrix */}
        ...
        {/* Detailed Transaction Ledger (Revenue Management) */}
