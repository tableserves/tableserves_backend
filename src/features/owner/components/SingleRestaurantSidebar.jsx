...
const SingleRestaurantSidebar = ({ sidebarOpen, setSidebarOpen, isMobile }) => {
  ...
  useEffect(() => {
    const refreshProfile = () => setRestaurantProfile(getRestaurantProfile());
    refreshProfile();

    const handleProfileUpdate = (event) => {
      if (event.detail.restaurantId === restaurantId) refreshProfile();
    };
  ...
  const menuSections = [
    ...
    },
    {
      ...
      icon: FaUtensils,
      items: [
        { path: `/restaurant/${restaurantId}/menu/categories`, label: 'Categories', icon: FaClipboardList },
        { path: `/restaurant/${restaurantId}/menu/items`, label: 'Menu Items', icon: FaUtensils },
      ]
    },
    {
      id: 'qr',
    ...
    },
    {
      ...
      icon: FaShoppingCart,
      items: [
        { path: `/restaurant/${restaurantId}/orders/live`, label: 'Live Orders', icon: FaShoppingCart },
        { path: `/restaurant/${restaurantId}/orders/history`, label: 'Order History', icon: FaHistory },
      ]
    },
    {
      id: 'analytics',
        ...
        <div className="flex items-center w-full">
          <div className="w-10 h-10 shrink-0 rounded-xl overflow-hidden bg-theme-bg-tertiary border border-theme-border-primary flex items-center justify-center">
            {restaurantProfile?.profilePicture ? (
              <img
                ...
                alt={restaurantProfile?.restaurantName || 'Restaurant'}
          ...
          >
            <h2 className="text-sm font-bold text-theme-text-primary truncate">
              {restaurantProfile?.restaurantName || user?.restaurantName || 'Dashboard'}
            </h2>
            <p className="text-xs font-medium text-theme-text-secondary uppercase tracking-wider">Owner Portal</p>
          </div>
        </div>
      </div>
