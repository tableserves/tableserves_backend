...
const VendorCredentials = () => {
  ...
  const inputClass = "w-full bg-theme-bg-primary border border-theme-border text-theme-text-primary rounded-xl px-4 py-2.5 font-raleway shadow-sm focus:outline-none focus:ring-2 focus:ring-theme-accent-primary/40 focus:border-theme-accent-primary transition-all text-sm";
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-12">
          ...
          <div className="admin-card p-6 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5 border border-theme-border shadow-sm">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl font-raleway font-bold flex items-center justify-center gap-2 bg-theme-bg-secondary border border-theme-border text-theme-text-primary hover:bg-theme-border transition-colors shadow-sm w-full sm:w-auto shrink-0"
            >
          ...
          <div className="flex flex-col gap-3">
            {filteredVendors.length === 0 && !error ? (
              ...
              filteredVendors.map((vendor) => {
                const isEditing = editingCredentials === vendor.id;
                ...
                return (
                  <motion.div
                    ...
                    className="admin-card rounded-xl border border-theme-border shadow-sm hover:shadow-md hover:border-theme-accent-primary/40 transition-all duration-300 flex flex-col p-4 bg-theme-bg-primary overflow-hidden"
                  >
                    {/* Top Row: Info & Actions */}
                    <div className={`flex flex-col md:flex-row items-start md:items-center justify-between gap-4 w-full ${isEditing ? 'pb-4 border-b border-theme-border' : ''}`}>
                      
                      {/* Left Side: Icon & Main Details */}
                      <div className="flex items-center gap-4 flex-1 min-w-0 w-full md:w-auto">
                        <div className="w-12 h-12 shrink-0 bg-theme-accent-primary/10 rounded-lg flex items-center justify-center border border-theme-accent-primary/20 overflow-hidden">
                          {vendor.logo ? (
                            <img src={vendor.logo} alt={vendor.name} className="w-full h-full object-cover" />
                          ) : (
                            <FaStore className="text-theme-accent-primary text-lg" />
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="text-base font-fredoka text-theme-text-primary truncate">{vendor.name}</h3>
                            <span className={`shrink-0 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${getStatusColor(vendor.status)}`}>
                              {vendor.status}
                            </span>
                          </div>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <FaUser className="text-theme-text-tertiary text-[10px]" />
                            <p className="text-xs font-raleway font-semibold text-theme-text-secondary truncate">{vendor.ownerName}</p>
                          </div>
                        </div>
                      </div>

                      {/* Right Side / Middle: Username & Action Button (Only if NOT editing) */}
                      {!isEditing && (
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between sm:justify-end gap-4 w-full md:w-auto pt-3 md:pt-0 border-t md:border-t-0 border-theme-border/50 shrink-0">
                          
                          {/* Username Block */}
                          <div className="flex flex-col gap-1 w-full sm:w-[220px]">
                            <span className="text-[10px] font-bold text-theme-text-tertiary uppercase tracking-wider">Username</span>
                            <div className="flex items-center justify-between bg-theme-bg-secondary/50 border border-theme-border rounded-lg px-3 py-1.5 h-[36px]">
                              <span className="font-mono text-sm text-theme-text-primary font-semibold truncate mr-2">
                                {vendor.loginCredentials?.username || <span className="text-theme-text-tertiary italic text-xs">Not configured</span>}
                              </span>
                              {vendor.loginCredentials?.username && (
                                <button
                                  ...
                                  className="text-theme-text-tertiary hover:text-theme-accent-primary transition-all p-1.5 rounded-md hover:bg-theme-bg-primary shrink-0"
                                  title="Copy username"
                                >
                                  {copiedField === `username-${vendor.id}` ? (
                                    <FaCheck className="text-status-success animate-pulse text-sm" />
                                  ) : (
                          ...
                          <button
                            ...
                            className="w-full sm:w-auto px-4 h-[36px] mt-0 sm:mt-[18px] rounded-lg text-sm font-bold flex items-center justify-center gap-2 border border-theme-accent-primary/30 bg-theme-accent-primary text-white shadow-sm hover:bg-theme-accent-primary/90 transition-all shrink-0"
                          >
                    ...
                    {isEditing && (
                      <motion.div
                        ...
                        className="pt-4 mt-1"
                      >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 bg-theme-bg-secondary/30 p-5 rounded-xl border border-theme-border/50">
                          ...
                          <div className="md:col-span-2 lg:col-span-1">
                            ...
                            />
                            <p className="mt-2 text-xs text-theme-text-tertiary font-raleway">This is used by the vendor to log into their dashboard.</p>
                          </div>

                          {/* Password Edits (Stacked or Grid) */}
                            ...
                            <div>
                              ...
                              <div className="relative w-full">
                                ...
                                <button
                                  ...
                                  className="absolute right-3 top-1/2 -translate-y-1/2 z-20 p-2 text-theme-text-tertiary hover:text-theme-accent-primary transition-colors"
                                >
                          ...
                          {/* Password Requirements Guide (Full width if typing) */}
                          {credentialForm.password && (
                            <div className="md:col-span-2 p-4 bg-theme-bg-primary border border-theme-border rounded-xl space-y-3 shadow-sm">
                              <div className="flex items-start gap-2.5">
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-[11px] font-bold text-theme-text-primary uppercase tracking-wider mb-1">Security Requirements</p>
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-y-2 gap-x-2 pt-1">
                          ...
                          {/* Action Buttons */}
                          <div className="md:col-span-2 flex flex-col sm:flex-row justify-end gap-3 mt-2 pt-4 border-t border-theme-border/50">
                            <button
                              onClick={cancelEdit}
                              className="px-6 py-2.5 rounded-lg font-raleway font-bold text-theme-text-secondary border border-theme-border bg-theme-bg-primary hover:bg-theme-bg-secondary hover:text-theme-text-primary transition-colors flex items-center justify-center gap-2 text-sm"
                            >
                            ...
                            <button
                              ...
                              disabled={!canSave}
                              className={`px-8 py-2.5 rounded-lg font-raleway font-bold flex items-center justify-center gap-2 text-sm transition-all shadow-sm
