...
export const CartProvider = ({ children }) => {
  ...
  // Initialize cart with scoped localStorage key
  const [cartItems, setCartItems] = useState(() => {
    try {
      ...
      logger.error('Failed to load cart from localStorage', error, 'CartContext');
      return [];
    }
  });
  ...
  useEffect(() => {
    try {
      ...
      // Save metadata
      const metadataKey = `${cartKey}_metadata`;
      ...
      logger.debug('Cart persisted', {
        cartKey,
        itemCount: cartItems.length,
        totalQuantity: count
      }, 'CartContext');
    } catch (error) {
      logger.error('Failed to persist cart', error, 'CartContext');
    }
  }, [cartItems, cartKey]);
